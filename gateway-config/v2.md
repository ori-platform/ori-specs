...
| `device_ids`           | `string[]` | —       | Yes      | Runtime device IDs this gateway will serve. Must contain at least one entry. IDs must not contain MQTT separators/wildcards, auth delimiters, or control characters (general category `Cc`, which includes the NUL an MQTT topic cannot carry), and with the evidence courier enabled each MUST lie in the evidence routing domain of [`evidence-exchange/v2.md`](../evidence-exchange/v2.md): 1 to 128 Unicode scalar values, none of them a control character (general category `Cc`), a character with the Unicode `White_Space` property, `/`, `+`, or `#`. |
...
Every rule below is checked at load; a configuration that breaks one is refused
...
and `max_bytes`. Each share MUST hold the registration reserve of
...
the gateway's own record format encodes them. The three environment variable names are names,
not values, and none of the values they name appears in this file.
